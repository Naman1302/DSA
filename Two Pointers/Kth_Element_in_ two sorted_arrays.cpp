...
Let's suppose ( A > D ) which is false condition, then we have to move in direction where it becomes true ( A <= D ), so we can move pointer x to left 
becuase it will decrease A and condition ( A <= D ) may become true.

high = mid - 1.

Let's suppose ( B > C ) which is false condition, then we have to move in direction where it becomes true ( B <= C ),so we can move pointer y to left 
( indirectly pointer x to right, because x+y is always equal to k, if x moves toward right then y will move left and vice versa) so it will decrease B 
and condition ( B <= C ) may become true.
low = mid + 1.
...
Corner Condition :
if x == 0, then arr1[ x - 1 ] will not exist and as this can be valid partioning and we are always comparing A <= D, 
so to get condition always true (because valid partionining) we have set A as minimum possible value, A = INT_MIN
Same case y == 0, then arr2[ y - 1 ] will not exist and as this can be valid partioning and we are always comparing B <= C, 
so to get condition always true (because valid partionining) we have set B as minimum possible value, B = INT_MIN
Similary if x == n1, then arr1[ x ] will not exist and as this can be valid partioning and we are always comparing B <= C, 
so to get condition always true (because valid partionining) we have set C as maximum possible value, C = INT_MAX.
Similary if y == n2, then arr2[ y ] will not exist and as this can be valid partioning and we are always comparing A <= D, 
so to get condition always true (because valid partionining) we have set D as maximum possible value, D = INT_MAX.
*/
